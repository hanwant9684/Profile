...
async def view_settings(client, message):
    ...
    ac_val = (ac.get("json_value") if ac else None) or "Disabled"
    text = (
        "⚙️ **Current Settings**\n\n"
        f"📢 Force Sub: `{fs_val}`\n"
    ...
    try:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT telegram_id, username, full_name, premium_expiry_date FROM users WHERE role = 'premium' AND (premium_expiry_date IS NULL OR premium_expiry_date >= $1)",
                datetime.now()
            )
        ...
        for user_data in premium_users:
            ...
            username_str = f" (@{user_data['username']})" if user_data.get("username") else ""
